...
test("운영 lane/필터/리스트 행 클릭은 toast-only가 아니라 화면 상태와 사이드패널로 연결된다", async ({ page }) => {
  await page.goto("/branches");
  const main = page.locator("main");
  ...
  await expect(page.getByTestId("scr-092-row-detail-panel")).toContainText("구현 연결점");
});

test("공통 mock 알림 액션은 퍼블리싱 설명 사이드패널을 열지 않는다", async ({ page }) => {
  ...
  await expect(main.getByRole("button", { name: "등급", exact: true })).toBeVisible();
});
