  ...
  return (
    <aside className="sticky top-0 h-screen overflow-y-auto border-r bg-slate-950 px-4 py-5 text-white no-scrollbar">
      <div className="flex items-center gap-3 px-2">
        ...
        <div><div className="font-bold">Pando Admin</div><div className="text-xs text-slate-400">docs4/V1 D01~D03</div></div>
      </div>
    ...
    <div className="space-y-5">
      <section className="rounded-2xl border bg-white p-5 shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <div className="flex flex-wrap items-center gap-2"><Badge variant="info">{screen.id}</Badge><Badge variant="outline">{screen.feature}</Badge>{screen.policyPending && <Badge variant="warning">정책 확인 필요</Badge>}<Badge variant="secondary">{screen.source}</Badge></div>
      ...
      <section className="grid grid-cols-4 gap-3">
        {screen.metrics.map((metric) => (
          <Card key={metric.label} className="shadow-none"><CardHeader className="pb-2"><CardDescription>{metric.label}</CardDescription><CardTitle className="text-xl">{metric.value}</CardTitle></CardHeader><CardContent><p className="text-xs text-slate-500">{metric.hint}</p></CardContent></Card>
        ))}
          ...
          <Card className="shadow-none">
            ...
            <CardContent className="space-y-4">
              <div className="flex flex-wrap gap-2">
                {screen.filters.map((filter) => <Badge key={filter} variant="outline">{filter}</Badge>)}
                {role === "HQ_ADMIN" && <Badge variant="info">전 지점 필터</Badge>}
              </div>
              <Tabs defaultValue={screen.tabs[0] ?? "기본"}>
                ...
                {(screen.tabs.length ? screen.tabs.slice(0, 1) : ["기본"]).map((tab) => <TabsContent key={tab} value={tab}><DataPanel screen={screen} /></TabsContent>)}
              </Tabs>
            </CardContent>
          </Card>
          <Card className="shadow-none">
            <CardHeader><CardTitle>권한별 액션 큐</CardTitle><CardDescription>불가 액션은 숨기지 않고 검수 가능하도록 비활성/권한 필요 상태로 표시합니다.</CardDescription></CardHeader>
            <CardContent className="flex flex-wrap gap-2">
              {screen.primaryActions.map((action) => {
                const allowed = hasPermission(role, action.permission);
                return (
                  <Button key={action.label} data-dialog-id={action.dialogId} variant={action.danger ? "destructive" : allowed ? "default" : "outline"} disabled={!allowed} onClick={() => action.dialogId ? openDialog(action.dialogId) : notify(`${action.label} mock 처리 완료`)}>
...
function DataPanel({ screen }: { screen: ScreenDefinition }) {
  if (!screen.tableColumns.length) return <div className="rounded-lg border bg-slate-50 p-5 text-sm text-slate-600">테이블 없는 화면입니다.</div>;
  return (
    <div className="rounded-lg border">
      <div className="flex items-center gap-2 border-b bg-slate-50 p-3"><Search className="size-4 text-slate-400" /><Input placeholder="이름·연락처·상품명 통합 검색" className="max-w-sm bg-white" /><Button variant="outline" size="sm">전체 해제</Button></div>
      <Table>
        <TableHeader><TableRow>{screen.tableColumns.map((column) => <TableHead key={column}>{column}</TableHead>)}</TableRow></TableHeader>
        <TableBody>{screen.rows.map((row, index) => <TableRow key={index}>{screen.tableColumns.map((column) => <TableCell key={column}>{statusAwareValue(row[column] ?? "-")}</TableCell>)}</TableRow>)}</TableBody>
      </Table>
      <div className="flex items-center justify-between border-t px-3 py-2 text-xs text-slate-500"><span>1-4 of 128</span><span>페이지네이션 mock · 행 액션은 우측 DLG 버튼으로 검수</span></div>
    </div>
  );
}
  ...
  return (
    <Dialog open={Boolean(dialog)} onOpenChange={(open) => !open && onClose()}>
      {dialog && <DialogContent data-testid="runtime-dialog">
        ...
        {!allowed && <div className="rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-800"><Lock className="mr-2 inline size-4" />현재 역할에는 처리 권한이 없습니다. 검수용으로 입력 필드는 보이지만 확인 버튼은 비활성화됩니다.</div>}
        <div className="grid gap-3 sm:grid-cols-2">
          {dialog.components.map((component, index) => <div key={component} className="space-y-1"><Label>{component}</Label>{index % 3 === 2 ? <Textarea defaultValue={`${component} mock 입력`} /> : <Input defaultValue={`${component} mock`} />}</div>)}
        </div>
        <Separator />
        <div className="rounded-lg bg-slate-50 p-3 text-xs leading-5 text-slate-600">
          실제 저장·삭제·승인·발송·결제·환불·외부연동은 수행하지 않습니다. 버튼 클릭 시 로컬 toast 또는 화면 상태만 mock 변경합니다.
        </div>
        <DialogFooter>
          <Button variant="outline" onClick={onClose}>닫기</Button>
          <Button variant={dialog.destructive ? "destructive" : "default"} disabled={!allowed} onClick={() => { notify(`${dialog.id} ${dialog.title} mock 처리 완료`, dialog.policyPending ? "warning" : "success"); onClose(); }}>{dialog.policyPending ? "정책 확인 상태로 저장" : "확인"}</Button>
        </DialogFooter>
      </DialogContent>}
    </Dialog>
  );
